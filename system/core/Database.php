...
$db_config = [
    ...
    //Localhost
    "development" => [
        "host" => $_ENV['local.hostname'],
        "database" => $_ENV['local.db'],
        "username" => $_ENV['local.username'],
        "password" => $_ENV['local.password'],
        "charset" => $_ENV['local.charset'],
        "port" => $_ENV['local.port'],
        "socket" => $_ENV['local.socket']
    ],

    // Testing
    "staging"  =>   [
        "host" => $_ENV['test.hostname'],
        "database" => $_ENV['test.db'],
        "username" => $_ENV['test.username'],
        "password" => $_ENV['test.password'],
        "charset" => $_ENV['test.charset'],
        "port" => $_ENV['test.port'],
        "socket" => $_ENV['test.socket']
    ],
    //Server
    "production"  => [
        "host" => $_ENV['server.hostname'],
        "database" => $_ENV['server.db'],
        "username" => $_ENV['server.username'],
        "password" => $_ENV['server.password'],
        "charset" => $_ENV['server.charset'],
        "port" => $_ENV['server.port'],
        "socket" => $_ENV['server.socket']
    ]
];
...
class Database
{
    ...
    public function __construct()
    {
        ...
        if ($db_config['env'] == "development") {
            $this->config = $db_config['development'];
        } elseif ($db_config['env'] == "staging") {
            $this->config = $db_config['staging'];
        } elseif ($db_config['env'] == "production") {
            $this->config = $db_config['production'];
        } else {
            die("Environment must be either 'development' or 'staging' or 'production'.");
        }
