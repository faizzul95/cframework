...
class BladeOne
{
    ...
    public function showError($id, $text, $critic = false, $alwaysThrow = false)
    {
        ...
        if (($this->throwOnError || $alwaysThrow) && $critic === true) {
            throw new \RuntimeException("CanThink Framework Error [$id] $text");
        } else {
            echo "<div style='background-color: red; color: black; padding: 3px; border: solid 1px black;'>";
            echo "CanThink Framework Error [$id]:<br>";
            echo "<span style='color:white'>$text</span><br></div>\n";
            if ($critic) {
                die(1);
            }
            if ($this->throwOnError) {
                error_log("CanThink Framework Error [$id] $text");
            }
        }
        return '';
    }
    ...
    protected function compilecsrf($params = null)
    {
        $easyCSRF = csrfProvider();
        $csrf_token = $easyCSRF->getCurrentToken('csrf-token') ?? csrf_token();

        return "<input type='hidden' name='_token' value='{$csrf_token}' class='form-control' />";
    }
